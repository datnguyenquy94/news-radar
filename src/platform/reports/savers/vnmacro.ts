...
const log = createLogger("report:vnmacro");
...
export async function saveVnMacroReport(
  ...
  if (!data.fetchSuccess) {
    log.info({ lang }, "No Vietnam market data available, skipping report.");
    return;
  }

  log.info({ lang }, "Calling LLM for Vietnam macro dashboard...");
  try {
    ...
    if (digestRepo) {
      ...
      if (url) log.info(`Created Vietnam macro issue (${lang}): ${url}`);
    }
  } catch (err) {
